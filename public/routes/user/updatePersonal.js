...
router.post('/updatePersonal',function(request,response){
  // the current users info needs to pupulate the forms data in this case
    ...
    User.updateOne({_id: global.userId}, { $set: { firstName: request.body.firstName, lastName: request.body.lastName, profilePic: request.body.profilePic, email: request.body.email } } , function(error,success){
       ...
       global.profilePic = reques.body.profilePic;
